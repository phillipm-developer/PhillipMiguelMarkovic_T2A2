...
# Returns all records from guardians_children table - Includes their embedded User information
...
# Returns the guardian child information for the guardian id supplied as a RESTful parameter
@guardian_child_bp.route('/<int:guardian_child_id>', methods=['GET'])
@jwt_required()
def one_guardian_child(guardian_child_id):
    # select * from guardian_child where is = guardian_child_id
...
# Create - CRUD route for creating a new guardian child entry in the database. 
...
def create_guardian_child():
    try:
        ...
        # insert into guardian_children (guardian_id, child_id, relationship_id) values (..., ..., ...)
        ...
        # Add and commit the new guardian child entry
